...
class Settings:
    ...
    ustx_wav_path: str = field(default="")  # Path to USTX WAV file
    ustx_path: str = field(default="")  # Path to USTX project file
    confidence_threshold: float = field(default=0.8)  # Confidence threshold for pitch detection
    vocal_track: int = field(default=1)  # Vocal track number
    time_align_radius: int = field(default=1)  # Time alignment radius
    semitone_shift: int = field(default=99)  # Semitone Shift
    pitch_align_smooth: int = field(default=1)  # Pitch alignment smoothing value
    scaler: float = field(default=2.5)  # Scaler for pitch delta
...
def add_cuda_path():
    ...
    for package in [nvidia.cuda_runtime, nvidia.cudnn, nvidia.cublas,
...
class PitchLoaderGUI:
    ...
    def create_widgets(self):
        ...
        file_inputs = [
            ...
            (_("USTX WAV Path:"), "ustx_wav_path", [(_("WAV Files"), "*.wav")], _("WAV file exported from OpenUtau, corresponding to the reference WAV.")),
        ...
        num_inputs = {
            _("USTX Info."): [
                (_("Vocal Track:"), "vocal_track", int, _("Index of the vocal track to process (starting from 1).")),
            ],
            _("Pitch Extraction"): [
                (_("Confidence Threshold:"), "confidence_threshold", float, _("Threshold for filtering uncertain pitch values.")),
            ],
            _("Time Alignment"): [
    ...
    def run_pitchloader(self):
        ...
        def task():
            try:
                ...
                # Step 2: Extract USTX WAV pitch
                self.update_status(_("Extracting pitch from USTX WAV..."))
                vocal_pitch, vocal_tick = pitchloader.get_wav_pitch_tick(
                    self.settings.ustx_wav_path,
                    tempo=ustx_tempo,
                    confidence_threshold=self.settings.confidence_threshold
                )
                self.progress_bar.step(10)

                # Step 3: Extract Reference WAV pitch
                self.update_status(_("Extracting pitch from Reference WAV..."))
                wav_pitch, wav_tick = pitchloader.get_wav_pitch_tick(
                ...
                seqs_to_plot = [(_("Ref. WAV Pitch"), wav_tick, wav_pitch, 'b'),
                                (_("USTX WAV Pitch"), vocal_tick, vocal_pitch, 'r')]
                ...
                # Step 4: Unify sequence time
                self.update_status(_("Unifying sequence time..."))
                unified_tick, unified_wav_pitch, unified_vocal_pitch = \
                    pitchloader.unify_sequence_time(
                        wav_tick,
                        wav_pitch,
                        vocal_tick,
                        vocal_pitch
                ...
                # Step 5: Align sequence time
                self.update_status(_("Aligning sequence time..."))
                time_aligned_wav_pitch = pitchloader.align_sequence_time(
                    unified_tick,
                    unified_wav_pitch,
                    unified_vocal_pitch,
                    self.settings.time_align_radius
                )
                self.progress_bar.step(10)

                # Step 6: Align sequence pitch
                ...
                time_pitch_aligned_wav_pitch, estimated_shift = pitchloader.align_sequence_pitch(
                ...
                seqs_to_plot = [(_("Aligned Ref. WAV Pitch"), unified_tick, time_pitch_aligned_wav_pitch, 'b'),
                                (_("Aligned USTX WAV Pitch"), unified_tick, unified_vocal_pitch, 'r'),
